...
## 🎬 [방법 B] 녹화 영상 서버 (복사 불필요)
rsync 복사 대신, NUC가 영상을 HTTP로 직접 제공합니다.
감지기와 함께 영상 서버도 띄우세요 (별도 터미널):
```bash
python3 recording_server.py    # 포트 8600에서 영상 제공
```
- 4070 대시보드가 http://192.168.0.65:8600 에서 영상을 직접 읽습니다.
- rsync, SSH 키 등록 전부 필요 없습니다.
- 감지기(intrusion_detector.py)와 영상서버(recording_server.py) 둘 다 실행하면 됩니다.
